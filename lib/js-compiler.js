...
function compileToJS(schema, defs, schemaMap) {
  ...
  // Exception: $dynamicRef is handled by the interpretive path even though codegen can't
  if (!defs && !codegenSafe(schema, schemaMap)) {
    const str = JSON.stringify(schema)
    if (!str.includes('"$dynamicRef"') && !str.includes('"$dynamicAnchor"')) return null
  }
  ...
  // $dynamicRef — resolve via anchor defs
  if (schema.$dynamicRef) {
    const anchorName = schema.$dynamicRef.startsWith('#') ? schema.$dynamicRef : '#' + schema.$dynamicRef
...
function codegenSafe(schema, schemaMap) {
  ...
  if (schema.$dynamicRef) return false
  ...
  if (schema.$ref) {
    ...
    // If the resolved target contains $dynamicRef, only allow codegen when the root schema
    // has unevaluated keywords (which need codegen). Otherwise bail to interpretive path.
    if (resolvedTarget && JSON.stringify(resolvedTarget).includes('"$dynamicRef"')) {
      if (schema.unevaluatedProperties === undefined && schema.unevaluatedItems === undefined) return false
    }
...
function compileToJSCodegen(schema, schemaMap) {
  ...
  // Build anchors map for $dynamicRef resolution
  const anchors = {}
  // Root schema's own $dynamicAnchor
  if (schema.$dynamicAnchor) {
    anchors['#' + schema.$dynamicAnchor] = schema
  }
  // Anchors from $defs
  if (rootDefs) {
    for (const def of Object.values(rootDefs)) {
      if (def && typeof def === 'object' && def.$dynamicAnchor) {
        anchors['#' + def.$dynamicAnchor] = def
      }
    }
  }
  ...
  if (schemaMap) {
    for (const ext of schemaMap.values()) {
      if (ext && typeof ext === 'object' && ext.$dynamicAnchor) {
        // Dynamic scope: root schema's anchor takes precedence (already set above)
        if (!anchors['#' + ext.$dynamicAnchor]) {
          anchors['#' + ext.$dynamicAnchor] = ext
        }
      }
    }
  }

  const ctx = { varCounter: 0, helpers: [], helperCode: [], closureVars: [], closureVals: [], rootDefs, refStack: new Set(), schemaMap: schemaMap || null, anchors }
  const lines = []
...
function genCode(schema, v, lines, ctx, knownType) {
  ...
  if (schema.$ref) {
    ...
    } else if (!m && schema.$ref.startsWith('#') && !schema.$ref.startsWith('#/') && ctx.rootDefs) {
      // Anchor ref: "#foo"
      const entry = ctx.rootDefs[schema.$ref]
      if (entry && entry.raw) {
        if (ctx.refStack.has(schema.$ref)) { if (!hasSiblings) return }
        else {
          ctx.refStack.add(schema.$ref)
          genCode(entry.raw, v, lines, ctx, knownType)
          ctx.refStack.delete(schema.$ref)
          if (!hasSiblings) return
        }
      }
    } else if (ctx.schemaMap) {
  ...
  // $dynamicRef — resolve via anchors map using closure-based validation
  if (schema.$dynamicRef) {
    const anchorKey = schema.$dynamicRef.startsWith('#') ? schema.$dynamicRef : '#' + schema.$dynamicRef
    if (ctx.anchors && ctx.anchors[anchorKey]) {
      // Compile the anchor target into a validator function and call it via closure
      // This avoids scope issues with deferred checks and circular inlining
      if (!ctx._dynRefVars) ctx._dynRefVars = {}
      if (!ctx._dynRefVars[anchorKey]) {
        const vi = ctx.varCounter++
        const varName = `_dr${vi}`
        ctx.closureVars.push(varName)
        // Lazy compilation: the anchor might reference itself recursively
        const target = ctx.anchors[anchorKey]
        const sm = ctx.schemaMap
        let compiled = null
        ctx.closureVals.push((d) => {
          if (!compiled) {
            compiled = compileToJSCodegen(target, sm) || compileToJS(target, null, sm) || (() => true)
          }
          return compiled(d)
        })
        ctx._dynRefVars[anchorKey] = varName
      }
      const fnVar = ctx._dynRefVars[anchorKey]
      lines.push(`if(!${fnVar}(${v}))return false`)
    }
  }
...
function compileToJSCodegenWithErrors(schema, schemaMap) {
  ...
  if (eRootDefs) {
    for (const def of Object.values(eRootDefs)) {
      if (def && typeof def === 'object' && def.$dynamicAnchor) eAnchors['#' + def.$dynamicAnchor] = def
    }
  }
  if (schemaMap) {
    for (const ext of schemaMap.values()) {
      if (ext && typeof ext === 'object' && ext.$dynamicAnchor && !eAnchors['#' + ext.$dynamicAnchor]) eAnchors['#' + ext.$dynamicAnchor] = ext
    }
  }

  const ctx = { varCounter: 0, helperCode: [], rootDefs: eRootDefs, refStack: new Set(), schemaMap: schemaMap || null, anchors: eAnchors }
  const lines = []
...
function genCodeE(schema, v, pathExpr, lines, ctx, schemaPrefix) {
  ...
  if (schema.$ref) {
    ...
    // Anchor ref: "#foo"
    if (!m && schema.$ref.startsWith('#') && !schema.$ref.startsWith('#/') && ctx.rootDefs) {
      const entry = ctx.rootDefs[schema.$ref]
      if (entry && entry.raw) {
        if (ctx.refStack.has(schema.$ref)) return
        ctx.refStack.add(schema.$ref)
        genCodeE(entry.raw, v, pathExpr, lines, ctx, schemaPrefix)
        ctx.refStack.delete(schema.$ref)
        return
      }
    }
  ...
  if (schema.$dynamicRef) {
    const anchorKey = schema.$dynamicRef.startsWith('#') ? schema.$dynamicRef : '#' + schema.$dynamicRef
    if (ctx.anchors && ctx.anchors[anchorKey]) {
      if (!ctx.refStack.has('$dynamicRef:' + anchorKey)) {
        ctx.refStack.add('$dynamicRef:' + anchorKey)
        genCodeE(ctx.anchors[anchorKey], v, pathExpr, lines, ctx, schemaPrefix)
        ctx.refStack.delete('$dynamicRef:' + anchorKey)
      }
    }
  }
...
function compileToJSCombined(schema, VALID_RESULT, schemaMap) {
  ...
  if (cRootDefs) {
    for (const def of Object.values(cRootDefs)) {
      if (def && typeof def === 'object' && def.$dynamicAnchor) cAnchors['#' + def.$dynamicAnchor] = def
    }
  }
  if (schemaMap) {
    for (const ext of schemaMap.values()) {
      if (ext && typeof ext === 'object' && ext.$dynamicAnchor && !cAnchors['#' + ext.$dynamicAnchor]) cAnchors['#' + ext.$dynamicAnchor] = ext
    }
  }

  const ctx = { varCounter: 0, helperCode: [], closureVars: [], closureVals: [],
                rootDefs: cRootDefs, refStack: new Set(), schemaMap: schemaMap || null, anchors: cAnchors }
  const lines = []
...
function genCodeC(schema, v, pathExpr, lines, ctx, schemaPrefix) {
  ...
  if (schema.$dynamicRef) {
    ...
    if (ctx.anchors && ctx.anchors[anchorKey]) {
      if (!ctx.refStack.has('$dynamicRef:' + anchorKey)) {
        ctx.refStack.add('$dynamicRef:' + anchorKey)
        genCodeC(ctx.anchors[anchorKey], v, pathExpr, lines, ctx, schemaPrefix)
        ctx.refStack.delete('$dynamicRef:' + anchorKey)
      }
    }
  }
